...
from .conftest import COMMENT_TEXT, NEW_COMMENT_TEXT, FORM_DATA, BAD_WORDS_FORM_DATA
...
def test_user_cant_use_bad_words(not_author_client, news, news_detail_url):
    response = not_author_client.post(news_detail_url, data=BAD_WORDS_FORM_DATA)
    assertFormError(
        response,
        form='form',
        field='text',
